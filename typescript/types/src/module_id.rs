...
pub struct Generator {
    cache: Arc<Mutex<(u32, FxHashMap<FileId, ModuleId>)>>,
}

impl Generator {
    /// Returns `(true, id)` if it's generated, and returns `(false, id)` if
    /// it's found from cache.
    pub fn generate(&self, file: FileId) -> (bool, ModuleId) {
        let mut cache = self.cache.lock();
        if let Some(v) = cache.1.get(&file) {
            return (false, *v);
        }
        cache.0 += 1;
        let module_id = ModuleId(cache.0);
        let res = cache.1.insert(file.clone(), module_id);
